...
export class EditarProductoComponent implements OnInit {
  ...
  ngObtenerProducto() {
    ...
    this._productoService.getOneProductoApi(id_producto).subscribe(data => {
      if(data !== null) {
        this.dataSelect = data.tipo_producto;
        ...
        this.valuesSelect = [
          {valor: 'alimentos', valorView: this.dataSelect},
        ...
        this.editarForm = this._formBuilder.group(
          ...
          { 
            id_producto: [dataFormPost.id_producto || 0],
            tipo_producto: [this.capitalizarPalabras(dataFormPost.tipo_producto), Validators.required],
  ...
  onSubmit() {
    ...
    if (this.editarForm.valid)  {
      ...
      const dataFormPut: Producto = {
        ...
        tipo_producto: this.editarForm.value.tipo_producto.toUpperCase(),
      ...
      this._productoService.putOneProductoApi(dataFormPut.id_producto,dataFormPut).subscribe(data => {
        // Recorremos el objeto, creando dos variables (clave y valor) y con Object.entries() pasamos como parámetro el objeto.
          for(let [c, v] of Object.entries(data)){
            console.log(v)
            if(v === null){
              this._generalService.mensajeTemporalError("¡El producto ya existe!")
            }else{
              this._generalService.mensajeConfirmacion("¿Desea editar este producto?", () => {
                this._generalService.mensajeTemporalCorrecto("¡Producto editado correctamente!");
